...
DECLARE @site_Out_International_EGP_query  VARCHAR(max) -- main query
...
-- Main query
SET @site_Out_International_EGP_query = '
				SELECT * INTO Total_Out_International_EGP_With_Technology_Pivot_Table
				FROM 
				(SELECT Site_Code_DWH, Status, ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Code_DWH, Status, Site_Date, Out_International_EGP FROM Detailed_Sites_Revenue_With_Technology_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Out_International_EGP)
                    FOR Site_Date IN ('+ @pivot_columns +')
                ) AS outInternationalEgpPivotTable)tmp'
...
if object_ID('Total_Out_International_EGP_With_Technology_Pivot_Table') IS NOT NULL
	DROP TABLE Total_Out_International_EGP_With_Technology_Pivot_Table;

EXEC (@site_Out_International_EGP_query)
