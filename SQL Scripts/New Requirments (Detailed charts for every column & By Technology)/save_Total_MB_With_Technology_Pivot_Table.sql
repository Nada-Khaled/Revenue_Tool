...
DECLARE @site_Total_MB_query  VARCHAR(max) -- main query
...
-- Main query
SET @site_Total_MB_query = '
				SELECT * INTO Total_MB_With_Technology_Pivot_Table
				FROM 
				(SELECT Site_Code_DWH, Status, ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Code_DWH, Status, Site_Date, Total_MB FROM Detailed_Sites_Revenue_With_Technology_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Total_MB)
                    FOR Site_Date IN ('+ @pivot_columns +')
                ) AS totalMbRevPivotTable)tmp'
...
if object_ID('Total_MB_With_Technology_Pivot_Table') IS NOT NULL
	DROP TABLE Total_MB_With_Technology_Pivot_Table;

EXEC (@site_Total_MB_query)
