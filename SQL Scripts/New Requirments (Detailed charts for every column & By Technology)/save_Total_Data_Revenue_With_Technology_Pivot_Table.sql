...
DECLARE @site_data_revenue_query  VARCHAR(max) -- main query
...
-- Main query
SET @site_data_revenue_query = '
				SELECT * INTO Total_Data_Revenue_With_Technology_Pivot_Table
				FROM 
				(SELECT Site_Code_DWH, Status, ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Code_DWH, Status, Site_Date, Total_Data FROM Detailed_Sites_Revenue_With_Technology_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Total_Data)
                    FOR Site_Date IN ('+ @pivot_columns +')
                ) AS dataRevPivotTable)tmp'

--PRINT(@pivot_columns)
--PRINT(@site_data_revenue_query)
--GO


if object_ID('Total_Data_Revenue_With_Technology_Pivot_Table') IS NOT NULL
	DROP TABLE Total_Data_Revenue_With_Technology_Pivot_Table;

EXEC (@site_data_revenue_query)
