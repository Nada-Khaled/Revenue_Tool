...
DECLARE @site_Roam_Data_MB_revenue_query  VARCHAR(max) -- main query
...
-- Main query
SET @site_Roam_Data_MB_revenue_query = '
				SELECT * INTO Total_Roam_Data_MB_With_Technology_Pivot_Table
				FROM 
				(SELECT Site_Code_DWH, Status, ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Code_DWH, Status, Site_Date, Roam_Data_MB FROM Detailed_Sites_Revenue_With_Technology_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Roam_Data_MB)
                    FOR Site_Date IN ('+ @pivot_columns +')
                ) AS totalRoamDataMbPivotTable)tmp'
...
if object_ID('Total_Roam_Data_MB_With_Technology_Pivot_Table') IS NOT NULL
	DROP TABLE Total_Roam_Data_MB_With_Technology_Pivot_Table;

EXEC (@site_Roam_Data_MB_revenue_query)
