...
DECLARE @network_total_revenue_query  VARCHAR(max) -- main query
...
-- Main query
SET @network_total_revenue_query = '
				SELECT * INTO Network_Total_Revenue_Pivot_Table
				FROM 
				(SELECT ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Date, Total_Revenue FROM Enhanced_Network_Revenue_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Total_Revenue)
                    FOR Site_Date IN ('+ @pivot_columns +')
                ) AS networkTotalRevPivotTable)tmp'

--PRINT(@pivot_columns)
--PRINT(@network_total_revenue_query)
--GO


if object_ID('Network_Total_Revenue_Pivot_Table') IS NOT NULL
	DROP TABLE Network_Total_Revenue_Pivot_Table;

EXEC (@network_total_revenue_query)
