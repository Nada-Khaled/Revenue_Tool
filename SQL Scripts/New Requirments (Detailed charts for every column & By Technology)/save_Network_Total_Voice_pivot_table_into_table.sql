...
DECLARE @network_voice_revenue_query  VARCHAR(max) -- main query
...
-- Main query
SET @network_voice_revenue_query = '
				SELECT * INTO Network_Voice_Revenue_Pivot_Table
				FROM 
				(SELECT ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Date, Total_Voice FROM Enhanced_Network_Revenue_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Total_Voice)
                    FOR Site_Date IN ('+ @pivot_columns +')
                ) AS networkVoiceRevPivotTable)tmp'
...
if object_ID('Network_Voice_Revenue_Pivot_Table') IS NOT NULL
	DROP TABLE Network_Voice_Revenue_Pivot_Table;

EXEC (@network_voice_revenue_query)
