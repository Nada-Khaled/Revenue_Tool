...
DECLARE @site_voice_revenue_query  VARCHAR(max) -- main query
...
-- Main query
SET @site_voice_revenue_query = '
				SELECT * INTO Total_Voice_Revenue_With_Technology_Pivot_Table
				FROM 
				(SELECT Site_Code_DWH, Status, ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Code_DWH, Status, Site_Date, Total_Voice FROM Detailed_Sites_Revenue_With_Technology_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Total_Voice)
                    FOR Site_Date IN ('+ @pivot_columns +')
                ) AS voiceRevPivotTable)tmp'
...
if object_ID('Total_Voice_Revenue_With_Technology_Pivot_Table') IS NOT NULL
	DROP TABLE Total_Voice_Revenue_With_Technology_Pivot_Table;

EXEC (@site_voice_revenue_query)
