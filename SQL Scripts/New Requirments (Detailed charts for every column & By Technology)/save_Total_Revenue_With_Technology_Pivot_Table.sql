...
DECLARE @site_total_revenue_query  VARCHAR(max) -- main query
...
-- Main query
SET @site_total_revenue_query = '
				SELECT * INTO Total_Revenue_With_Technology_Pivot_Table
				FROM 
				(SELECT Site_Code_DWH, Status, ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Code_DWH, Status, Site_Date, Total_Revenue FROM Detailed_Sites_Revenue_With_Technology_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Total_Revenue)
...
--PRINT(@pivot_columns)
--PRINT(@site_total_revenue_query)
--GO


if object_ID('Total_Revenue_With_Technology_Pivot_Table') IS NOT NULL
	DROP TABLE Total_Revenue_With_Technology_Pivot_Table;

EXEC (@site_total_revenue_query)
