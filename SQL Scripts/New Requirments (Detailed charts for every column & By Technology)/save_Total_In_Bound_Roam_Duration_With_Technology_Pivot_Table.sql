...
if object_id('Detailed_Sites_Revenue_With_Technology_Chart','v') is not null
	Drop VIEW Detailed_Sites_Revenue_With_Technology_Chart

GO

-- Create the the view again to fill it with newly updated data:
CREATE VIEW Detailed_Sites_Revenue_With_Technology_Chart 
AS
	SELECT 
	...
	CAST(DWH_With_LAC_CI.[YEAR] AS nvarchar(4)) + '-' + RIGHT('0'+ CONVERT(VARCHAR, DWH_With_LAC_CI.[Month]), 2) AS 'Site_Date'

	,Site_Code_DWH
	,DWH_With_LAC_CI.[Status],
	...
	FROM DWH_With_LAC_CI

	GROUP BY DWH_With_LAC_CI.[Year], DWH_With_LAC_CI.[Month], Site_Code_DWH, DWH_With_LAC_CI.[Status]

--ORDER BY Year, Month, ShortCode, DWH_With_LAC_CI.[Status]
...
DECLARE @site_In_Bound_Roam_Duration_query  VARCHAR(max) -- main query
DECLARE @pivot_columns VARCHAR(8000) -- list to hold dynamically pivot columns

SET @pivot_columns =''

-- Get string columns
SELECT @pivot_columns += '[' + CONVERT(VARCHAR, Site_Date) +'],' FROM (SELECT DISTINCT Site_Date FROM Detailed_Sites_Revenue_With_Technology_Chart) AS pivotColList

-- delete the last comma
SET @pivot_columns = LEFT(@pivot_columns,LEN(@pivot_columns)-1)

-- Main query
SET @site_In_Bound_Roam_Duration_query = '
				SELECT * INTO Total_In_Bound_Roam_Duration_With_Technology_Pivot_Table
				FROM 
				(SELECT Site_Code_DWH, Status, ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Code_DWH, Status, Site_Date, In_Bound_Roam_Duration FROM Detailed_Sites_Revenue_With_Technology_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(In_Bound_Roam_Duration)
                    FOR Site_Date IN ('+ @pivot_columns +')
                ) AS inBoundRoamDurationPivotTable)tmp'
...
if object_ID('Total_In_Bound_Roam_Duration_With_Technology_Pivot_Table') IS NOT NULL
	DROP TABLE Total_In_Bound_Roam_Duration_With_Technology_Pivot_Table;

EXEC (@site_In_Bound_Roam_Duration_query)
