...
SET @site_roaming_data_revenue_query = '
				SELECT * INTO Roaming_Data_Revenue_With_Technology_Pivot_Table
				FROM 
				(SELECT Site_Code_DWH, Status, ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Code_DWH, Status, Site_Date, Roaming_Data FROM Detailed_Sites_Revenue_With_Technology_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Roaming_Data)
...
if object_ID('Roaming_Data_Revenue_With_Technology_Pivot_Table') IS NOT NULL
	DROP TABLE Roaming_Data_Revenue_With_Technology_Pivot_Table;
