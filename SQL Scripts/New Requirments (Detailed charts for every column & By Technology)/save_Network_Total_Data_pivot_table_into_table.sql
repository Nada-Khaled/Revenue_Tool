...
DECLARE @network_data_revenue_query  VARCHAR(max) -- main query
...
-- Main query
SET @network_data_revenue_query = '
				SELECT * INTO Network_Data_Revenue_Pivot_Table
				FROM 
				(SELECT ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Date, Total_Data FROM Enhanced_Network_Revenue_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Total_Data)
                    FOR Site_Date IN ('+ @pivot_columns +')
                ) AS networkDataRevPivotTable)tmp'

--PRINT(@pivot_columns)
--PRINT(@network_data_revenue_query)
--GO


if object_ID('Network_Data_Revenue_Pivot_Table') IS NOT NULL
	DROP TABLE Network_Data_Revenue_Pivot_Table;

EXEC (@network_data_revenue_query)
