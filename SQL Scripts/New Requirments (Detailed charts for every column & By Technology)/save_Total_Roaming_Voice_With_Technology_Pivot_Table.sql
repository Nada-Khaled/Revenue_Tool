...
DECLARE @site_roaming_voice_revenue_query  VARCHAR(max) -- main query
...
-- Main query
SET @site_roaming_voice_revenue_query = '
				SELECT * INTO Total_Roaming_Voice_Revenue_With_Technology_Pivot_Table
				FROM 
				(SELECT Site_Code_DWH, Status, ' + @pivot_columns +
				' FROM
                (
                  SELECT Site_Code_DWH, Status, Site_Date, Roaming_Voice FROM Detailed_Sites_Revenue_With_Technology_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Roaming_Voice)
                    FOR Site_Date IN ('+ @pivot_columns +')
                ) AS roamingVoiceRevPivotTable)tmp'

--PRINT(@pivot_columns)
--PRINT(@site_roaming_voice_revenue_query)
--GO


if object_ID('Total_Roaming_Voice_Revenue_With_Technology_Pivot_Table') IS NOT NULL
	DROP TABLE Total_Roaming_Voice_Revenue_With_Technology_Pivot_Table;

EXEC (@site_roaming_voice_revenue_query)
