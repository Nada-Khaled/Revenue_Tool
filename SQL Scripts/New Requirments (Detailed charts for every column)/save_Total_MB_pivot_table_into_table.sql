...
SET @site_Total_MB_query = '
				SELECT * INTO Total_MB_Pivot_Table
				FROM 
				(SELECT ShortCode, ' + @pivot_columns +
				' FROM
                (
                  SELECT ShortCode, Site_Date, Total_MB FROM Detailed_Sites_Revenue_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Total_MB)
...
if object_ID('Total_MB_Pivot_Table') IS NOT NULL
	DROP TABLE Total_MB_Pivot_Table;
