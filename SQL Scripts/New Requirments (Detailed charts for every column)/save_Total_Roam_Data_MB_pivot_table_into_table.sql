...
SET @site_Roam_Data_MB_revenue_query = '
				SELECT * INTO Total_Roam_Data_MB_Pivot_Table
				FROM 
				(SELECT ShortCode, ' + @pivot_columns +
				' FROM
                (
                  SELECT ShortCode, Site_Date, Roam_Data_MB FROM Detailed_Sites_Revenue_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Roam_Data_MB)
...
if object_ID('Total_Roam_Data_MB_Pivot_Table') IS NOT NULL
	DROP TABLE Total_Roam_Data_MB_Pivot_Table;
