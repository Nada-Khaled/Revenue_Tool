...
SET @site_Out_International_EGP_query = '
				SELECT * INTO Total_Out_International_EGP_Pivot_Table
				FROM 
				(SELECT ShortCode, ' + @pivot_columns +
				' FROM
                (
                  SELECT ShortCode, Site_Date, Out_International_EGP FROM Detailed_Sites_Revenue_Chart
                ) AS tmp_tbl             
                PIVOT
                (
...
if object_ID('Total_Out_International_EGP_Pivot_Table') IS NOT NULL
	DROP TABLE Total_Out_International_EGP_Pivot_Table;
