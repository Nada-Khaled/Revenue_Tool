...
SET @site_data_revenue_query = '
				SELECT * INTO Data_Revenue_Pivot_Table
				FROM 
				(SELECT ShortCode, ' + @pivot_columns +
				' FROM
                (
                  SELECT ShortCode, Site_Date, Total_Data FROM Detailed_Sites_Revenue_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Total_Data)
...
if object_ID('Data_Revenue_Pivot_Table') IS NOT NULL
	DROP TABLE Data_Revenue_Pivot_Table;
