...
SET @site_roaming_voice_revenue_query = '
				SELECT * INTO Roaming_Voice_Revenue_Pivot_Table
				FROM 
				(SELECT ShortCode, ' + @pivot_columns +
				' FROM
                (
                  SELECT ShortCode, Site_Date, Roaming_Voice FROM Detailed_Sites_Revenue_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Roaming_Voice)
...
if object_ID('Roaming_Voice_Revenue_Pivot_Table') IS NOT NULL
	DROP TABLE Roaming_Voice_Revenue_Pivot_Table;
