...
SET @site_In_Bound_Roam_Duration_query = '
				SELECT * INTO Total_In_Bound_Roam_Duration_Pivot_Table
				FROM 
				(SELECT ShortCode, ' + @pivot_columns +
				' FROM
                (
                  SELECT ShortCode, Site_Date, In_Bound_Roam_Duration FROM Detailed_Sites_Revenue_Chart
                ) AS tmp_tbl             
                PIVOT
                (
...
if object_ID('Total_In_Bound_Roam_Duration_Pivot_Table') IS NOT NULL
	DROP TABLE Total_In_Bound_Roam_Duration_Pivot_Table;
