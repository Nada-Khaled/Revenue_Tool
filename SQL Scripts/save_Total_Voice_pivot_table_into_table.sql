...
SET @site_voice_revenue_query = '
				SELECT * INTO Voice_Revenue_Pivot_Table
				FROM 
				(SELECT ShortCode, ' + @pivot_columns +
				' FROM
                (
                  SELECT ShortCode, Site_Date, Total_Voice FROM Enhanced_Sites_Revenue_Chart
                ) AS tmp_tbl             
                PIVOT
                (
                    SUM(Total_Voice)
...
if object_ID('Voice_Revenue_Pivot_Table') IS NOT NULL
	DROP TABLE Voice_Revenue_Pivot_Table;
