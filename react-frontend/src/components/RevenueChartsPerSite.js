...
export const RevenueChartsPerSite = () => {
  ...
  const HandleSelectTechnology = (evt) => {
    setSiteCodeAndTechnologyChosen(siteCodeAndTechnologyChosen + 1);
    setSelectedTechnology(evt.target.value);
  ...
  const HandleSelectSiteCode = (evt) => {
    setSiteCodeAndTechnologyChosen(siteCodeAndTechnologyChosen + 1);
    setSelectedSiteCode(evt.target.value);
  };

  // const HandleSelectSiteCode_ = (evt) => {
  ...
  useEffect(() => {
    ...
    // to run only once, when rendered
    if (siteCodeAndTechnologyChosen === 0) {
      console.log("once when page is loaded");
      trackPromise(
        axios
          ...
          .then((response) => {
            ...
            // console.log("^^^^^ site codes type:");
            // console.log(typeof(siteCodesList));
            setAreSiteCodesAvailable(true);
          })
      );
    }
    // to ensure that technology and siteCode are BOTH chosen
    else if (siteCodeAndTechnologyChosen > 1) {
      // console.log("each time the counter changes");
      setAreSiteCodeAndTechnologyChosen(true);

      // console.log("siteCodeAndTechnologyChosen");
      // console.log(siteCodeAndTechnologyChosen);

      axios
        .get(
          global.config.BACKEND_IP +
            "/api/showChartBySiteAndTechnology/" +
            selectedSiteCode +
            "/" +
            selectedTechnology
        )
        ...
        <React.Fragment>
          <Grid container style={{ marginTop: 30 }}>
            ...
            >
              ...
              >
                <option value="no_technology_selected" key="no_technology_selected"></option>
